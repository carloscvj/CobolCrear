 ...
 * @author carlos
 */
public class CrearCobol extends Task implements Serializable {
        ...
        for (FileSet unF : getFilesets()) {
            ...
            while (iter.hasNext()) {
                ...
                try {
                    ...
                    hac.run(uno.getFile(), getBaseDir(), getCualHago(), getCarpetaSrc(), getCarpetaLyb()); //ste se encarga de mirar los lastModified de los ficheros.
                } catch (Exception ex) {
                    log(ex.toString(), Project.MSG_ERR);
                }
            }
        }
        log("-------------------------fin generando fuentes COBOL-------------------------------");
    }
